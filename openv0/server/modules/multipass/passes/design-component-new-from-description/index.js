...
async function run(req) {
  ...
  const stream = await replitai.chat({
    ...
    messages: [
      {
        author: 'system',
        content: `Your task is to design a new ${req.query.framework} component for a web app, according to the user's request.\n` +
          `If you judge it is relevant to do so, you can specify pre-made library components to use in the task.\n` +
          `You can also specify the use of icons if you see that the user's request requires it.` +
          `\n---\n` +
          "Multiple library components can be used while creating a new component in order to help you do a better design job, faster.\n\nAVAILABLE LIBRARY COMPONENTS:\n```\n" +
          ...
          JSON.stringify(
            ...
            , null, `\t`) +
          `\n- User task : ${req.query.description}` +
          `\n\nDesign the new ${req.query.framework} web component task for the user as the creative genius you are. Your answer should start and end with curly brackets.`,
      },
    ],
  });
